  ...
  const handleDelete = async (id) => {
    ...
    try {
      const endpoint = source === "category"
        ? `/admin/category-sample-downloads/${id}`
        : `/admin/Downloadsample/${id}`;
    ...
    <div className="truncate text-xs text-gray-700">
      ...
      <td className="px-6 py-4">
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5  text-red-500 rounded-lg transition-colors cursor-pointer"
          title="Delete record"
        >
          <Trash size={14} />
        </button>
